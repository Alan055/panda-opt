...
<template>
    <div class="startFigure">
        <transition name="el-zoom-in-bottom">
            <div>
                <el-tabs type="border-card">
                    <el-tab-pane label="banner管理">
                        <div class="plugins-tips">
                            ...
                            <div class="add-btn" @click="addPopupDialogOpen">
                                <i class="el-icon-lx-roundadd"></i>新增Banner管理
                            </div>
                        </div>
                        <template>
                            <el-tabs class="second-tabs" tab-position="top" style="" v-model="allactiveName"
                                ...
                                <el-tab-pane v-for="item in BannerTypeList" :label="item.desc" :name="item.positionId"
                                    :key="item.positionId">
                                    <div>
                                        <div class="figure-title">Banner管理/全部-{{item.desc}}</div>
                                    </div>
                    ...
                    <el-tab-pane label="banner素材管理">
                        <div class="plugins-tips">
                            为保障用户体验，首页弹窗图统一为PNG格式，建议尺寸750*488，大小不超过1M。
                            <div class="add-btn" @click="addMaterialList">
                                <i class="el-icon-lx-roundadd"></i>新增banner素材
                            </div>
                        </div>
                        <div>
                            <div class="figure-title">banner素材素材</div>
                        </div>
                        <el-table stripe v-loading="loading" :data="dialogMaterialList.list || []" border
                            style="width: 100%">
                            <el-table-column type="index" label="序号">
                            </el-table-column>
        ...
        <el-dialog :title="dialogText" width="40%" center :visible.sync="addPopupDialog" @closed="closePopupDialog">
            <el-form :model="addPopupForm" :rules="addPopupFormRules" ref="addPopupForm" label-width="100px"
                ...
                <el-form-item label="位置" prop="positionid">
                    <el-select v-model="addPopupForm.positionid" autocomplete="off" placeholder="选择banner位置">
                        <el-option filterable v-for="item in BannerTypeList" :key="item.positionId" :label="item.desc"
        ...
        <el-dialog :title='materialText' width="40%" center :visible.sync="addMaterialListDialog" @closed="closeDialog">
            ...
            <el-form :model="addMaterialForm" :rules="addMaterialRules" ref="addMaterialForm" label-width="100px"
                ...
                <el-form-item label="素材名称" prop="icontext">
                    <el-input v-model="addMaterialForm.icontext" placeholder="请输入素材名称" autocomplete="off"></el-input>
                </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            handleStop(index, row) {
                ...
                this.$confirm('此操作将' + text + 'banner， 是否继续?', '提示', {
            ...
            addPopupDialogOpen() {
                this.dialogText = '添加banner管理';
                this.addPopupDialog = true;
            },
            //打开添加首页弹窗素材
            addMaterialList(item) {
                this.materialText = '添加banner素材';
                this.addMaterialListDialog = true;
            },
            //打开编辑首页弹窗素材
            editMaterial(item) {
                this.materialText = '编辑banner素材';
                this.addMaterialListDialog = true;
            ...
            //添加素材
            addMaterialItem() {
                if (this.addPopupForm.advertList && this.addPopupForm.advertList.length == 20) {
                    this.$message.error("最多添加20张素材")
                } else {
