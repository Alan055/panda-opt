...
<template>
    <div class="startFigure">
        <transition name="el-zoom-in-bottom">
            <div>
                <el-tabs type="border-card">
                    <el-tab-pane label="Icon管理">
                        <template>
                            <el-tabs class="second-tabs" tab-position="top" style="" v-model="allactiveName"
                                @tab-click="checkAllStatus">
                                <el-tab-pane v-for="item in BannerTypeList" :label="item.desc" :name="item.positionId"
                                    :key="item.positionId">
                                    <div class="plugins-tips">
                                        为保障用户体验，icon图统一为PNG格式，尺寸：80*80px，大小不超过1M。
                                        <div class="add-btn" @click="addPopupDialogOpen">
                                            <i class="el-icon-lx-roundadd"></i>新增Icon管理
                                        </div>
                                    </div>
                                    <div>
                                        <div class="figure-title">Icon管理</div>
                                    </div>
                                    <el-table v-loading="loading" :data="allDialogList.list" border stripe
                                        style="width: 100%">
                                        <el-table-column type="index" label="序号">
                                        </el-table-column>
                                        <el-table-column prop="positionid" label="位置" :formatter="filterPosition"
                                            width="180">
                                        </el-table-column>
                                        <el-table-column prop="userstatusname" width="200" label="用户状态">
                                        </el-table-column>
                                        ...
                                        <el-table-column label="ABtest" prop="openabtest" :formatter="formatAbtest"
                                            v-if="allactiveName == '1023002'">
                                        </el-table-column>
                    ...
                    <el-tab-pane label="Icon素材管理">
                        <div class="plugins-tips">
                            为保障用户体验，icon图统一为PNG格式，尺寸：80*80px，大小不超过1M。
                            <div class="add-btn" @click="addMaterialList">
                                <i class="el-icon-lx-roundadd"></i>新增Icon素材
                            </div>
                        </div>
                        <div>
                            <div class="figure-title">Icon素材素材</div>
                        </div>
                        <el-table v-loading="loading" :data="dialogMaterialList.list || []" border stripe
                            style="width: 100%">
                            <el-table-column type="index" label="序号">
                            </el-table-column>
                            <el-table-column prop="icontext" label="Icon配文">
                            </el-table-column>
                            <el-table-column prop="jumplink" label="链接">
                            </el-table-column>
                            ...
                            <el-table-column fixed="right" label="操作" width="150">
                                <template slot-scope="scope">
                                    ...
                                    <el-button @click="deletDetail(scope.row)" type="danger" size="mini">删除</el-button>
        ...
        <el-dialog :title="iconTitle" width="40%" center :visible.sync="addPopupDialog" @closed="closePopupDialog">
            <el-form :model="addPopupForm" :rules="addPopupFormRules" ref="addPopupForm" label-width="100px"
                ...
                <el-form-item label="位置" prop="positionid">
                    <el-select filterable v-model="addPopupForm.positionid" autocomplete="off" placeholder="选择Icon位置">
                        <el-option v-for="item in BannerTypeList" :key="item.positionId" :label="item.desc"
                            :value="item.positionId">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="ABtest：" v-if="addPopupForm.positionid == '1023002'">
                    <el-radio-group v-model="addPopupForm.openabtest">
                        <el-radio-button v-for="item in abOption" :label="item.value" :value='item.value'
                            :key="item.value">
                            {{item.label}}
                        </el-radio-button>
                    </el-radio-group>
                ...
                <el-form-item label="用户状态" prop="userstatus">
                    <el-select v-model="addPopupForm.userstatus" filterable autocomplete="off" placeholder="选择用户状态">
                ...
                <el-form-item labelWidth="0px">
                    <el-row v-for="(key,index) in addPopupForm.advertList" :key="index" :gutter="20"
                        ...
                        <el-col :span="18" class="dialog-advertItem-left">
                            <el-col :span="8" v-if="dialogMaterialListAll">
        ...
        <el-dialog :title="iconTitleMateril" width="40%" center :visible.sync="addMaterialListDialog"
            @closed="closeDialog">
            <div class="dialog-tips"><i class="el-icon-info"></i> 为保障用户体验，素材图统一为PNG、jpg、gif格式，建议尺寸80*80，大小不超过1M。</div>
            <el-form :model="addMaterialForm" :rules="addMaterialRules" ref="addMaterialForm" label-width="100px"
                size="mini">
                <el-form-item label="Icon配文" prop="icontext">
...
<script>
    ...
    export default {
        name: 'BannerManager',
        data: function () {
            const changeValidateInput = (rule, value, callback) => {
                if (value === '') {
                    callback(new Error('请输入Icon配文'));
                } else {
                    let len = 0;
                    ...
                    if (len > 8 || len < 4) {
                        callback(new Error('Icon配文长度为4-8个字符,注：一个中文为2个字符'));
                    } else {
                        callback();
                    }
        ...
        computed: {
            ...
            dialogMaterialList: function () {
                return this.$store.state.iconMaterialList;
            },
            ...
            dialogMaterialListAll: function () {
                return this.$store.state.iconMaterialListAll;
            },
            getAdvertImgurl: function () {
                return function (id) {
                    let list = this.$store.state.iconMaterialListAll.list;
                    if (list) {
        ...
        methods: {
            init() {
                this.getDialogList(this.allactiveName, this.userStatus);
                this.getBannerTypeList();
                this.$store.dispatch('changeGetUserStatusList');
                this.$store.dispatch('changeIconMaterialList', this.defaultParams);
            },
            ...
            handleStop(index, row) {
                ...
                this.$confirm('此操作将' + text + 'icon，是否继续?', '提示', {
            ...
            //获取所有bannerType;
            getBannerTypeList() {
                this.Api.maGetAllIconTypeList({}).then((res) => {
                    if (res.data.code == 200) {
            ...
            closePopupDialog() {
                ...
                Object.keys(this.addPopupForm).forEach((key) => {
                    ...
                    } else if (key == 'openabtest') {
                        this.addPopupForm.openabtest = 1;
            ...
            getDialogList(iconType) {
                let params = {
                    positionId: this.allactiveName || '1001004',
            ...
            addPopupDialogOpen() {
                this.iconTitle = '添加Icon';
                this.addPopupDialog = true;
            },
            //打开添加首页弹窗素材
            addMaterialList(item) {
                this.addMaterialListDialog = true;
                this.iconTitleMateril = '添加Icon素材';
            },
            //打开编辑首页弹窗素材
            editMaterial(item) {
                this.iconTitleMateril = '编辑Icon素材';
                this.addMaterialListDialog = true;
            ...
            sureSavePopup: function (formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        if (this.allactiveName == '1023002') {
                            params.openabtest = this.addPopupForm.openabtest;
                        }
                ...
                this.$confirm('此操作将永久删除Icon ，是否继续?', '提示', {
                    ...
                    this.Api.maDelBannerAndIcon({
                        ...
                        if (res.data.code == 200) {
                            ...
                            this.getDialogList(this.iosactiveName);
                        }
                    });
                }).catch(() => {
                    this.$message({
