            ...
            <div v-loading='formLoading'>
              ...
              <div class="content-form">
                ...
                <el-form ref="formData" :model="formData" :rules="formDataRules" label-width="100px">
                  <!--  输入Push ID-->
                  <el-form-item label="短信 ID：" prop="pushId">
                    <el-input
                      type="text"
                      placeholder="短信 ID"
                  ...
                  <el-form-item label="短信内容：">
                    <el-input
                      ...
                      disabled
                      placeholder="输入Push ID后自动填充Push内容"
                      v-model="formData.content"
                    >
                    </el-input>
                    <div :class='["contentLength",codeLength>=200?"redText":""]'>{{codeLength}}/200</div>
                  </el-form-item>
  ...
  export default {
    ...
    data() {
      var checkPushId = (rule, value, callback) => {
        let _this = this;
        if (!value) {
          return callback(new Error('短信 Id不能为空'));
        }

        if (value.length < 18) {
          callback(new Error('请输入正确长度的短信ID'));
        } else {
          ...
          api.msGetSysEventDetailById({
            ...
            if (res.data.code == 200) {
              if (res.data.data) {
                if (res.data.data.messagetype == '1') {
      ...
      return {
        ...
        tableOptions: [
          ...
          {
            label: '消息内容', prop: 'content', type: 'content'
          },
          {
    ...
    methods: {
      ...
      exportList() {
        ...
        let url = this.apiUrl + '/opt/smsmass/exportSmsMessageList?status=' + this.checkStatus + '&sendType=' + this.listPushObject + '&keyword=' + this.searchList;
      ...
      sendApply(formName) {
        ...
        if (valid_common && valid_common) {
          ...
          api.sentSaveBatchInfo(params).then((res) => {
            if (res.data.code == 200) {
