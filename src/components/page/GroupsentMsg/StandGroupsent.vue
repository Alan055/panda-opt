        ...
        <el-tabs type="border-card">
          <el-tab-pane label="新建群发">
            ...
            <div class="plugins-tips">
              为保障用户体验，站内信消息群发请务必严谨，下发前请先内部测试，确认无误后再对用户进行批量下发。晚上10:00至早上8:00禁止对用户下发。
            </div>
            <div v-loading="formLoading">
              ...
              <div class="content-form">
                ...
                <el-form ref="formData" :model="formData" :rules="formDataRules" label-width="100px">
                  <!--  输入站内信 ID-->
                  <el-form-item label="站内信 ID：" prop="pushId">
                    <el-input
                      type="text"
                      placeholder="站内信 ID"
                      v-model="formData.pushId">
                    </el-input>
                  </el-form-item>
                  <!-- 跳转方式 -->
                  <el-form-item label="跳转方式：">
                    <el-select disabled v-model="formData.jumpType" placeholder="跳转方式">
                      <el-option v-for="(item,index) in optionJumpType" :label="item.label" :value="item.value"
                                 :key="index"></el-option>
                    </el-select>
                  </el-form-item>

                  <!-- 跳转链接 -->
                  <el-form-item label="跳转链接：" v-if="formData.jumpType == 0 || formData.jumpType == 1">
                    <el-input
                      type="text"
                      disabled
                      placeholder="跳转链接"
                  ...
                  <el-form-item label="摘要图片：">
                    <img :src="formData.msgimgurl" width="150">
                  </el-form-item>
          ...
          <el-tab-pane label="群发列表" v-loading="exportLoading">
              ...
              <el-form :inline="true">
                <el-form-item label="状态：">
                  <el-radio-group v-model="checkStatus" @change="getCheckStatus">
                    <el-radio-button v-for="item in statusOption" :label="item.value" :value='item.value'
                                     :key="item.value">
                      {{item.label}}
                    </el-radio-button>
                  </el-radio-group>
                ...
                <el-form-item label="群发对象：">
                  <el-select v-model="listPushObject" placeholder="请选群发对象" @change="listPushObjectChange">
                    <el-option v-for="(item,index) in [{label:'全部',value:'0'}].concat(objectOption)" :label="item.label" :value="item.value"
                               :key="index"></el-option>
                  </el-select>
                </el-form-item>
              ...
              <el-form :inline="false">
                <el-form-item label="选择表格显示项">
                  ...
                  <el-checkbox-group v-model="checkedTables" @change="handleCheckedChange">
                    <el-checkbox v-for="(item,index) in tableOptions" checked :label="item" :key="index">
                      {{item.label}}
                    </el-checkbox>
                  </el-checkbox-group>
                </el-form-item>
            ...
            <el-table
              stripe
              v-loading="loading"
              :data="queryMassMessageMassList.list || []"
              border
              style="width: 100%">


              <!-- 中间可屏蔽项 -->
              <template v-for="col in tableOptions">
                <el-table-column
                  :prop="col.prop"
                  :label="col.label"
                  :key="col.label"
                  :type="col.type=='index'? 'index':''"
                  v-if="col.type == 'popover'"
                  :width="widthTh(col.type)"
                ...
                <el-table-column
                  ...
                  :key="col.label"
                  :type="col.type=='index'? 'index':''"
                  v-if="(col.type != 'img' && col.type != 'popover')"
                  :width="widthTh(col.type)"
                  :formatter="col.formatters"
                >
                </el-table-column>
            ...
            <el-pagination
              ...
              :total="queryMassMessageMassList.total">
            </el-pagination>
          </el-tab-pane>
        </el-tabs>
  ...
  export default {
    ...
    data() {
      var checkPushId = (rule, value, callback) => {
        let _this = this;
        if (!value) {
          return callback(new Error('站内信 Id不能为空'));
        }

        if (value.length < 18) {
          callback(new Error('请输入正确长度的站内信ID'));
        } else {
          ...
          api.msGetSysEventDetailById({
            ...
            if (res.data.code == 200) {
              if (res.data.data) {
                if (res.data.data.messagetype == '3') {
                    ...
                    if (res.data.code == 200) {
                      if (res.data.data) {
                        if (res.data.data.messagetype == '3') {
                          ...
                          this.formData.jumpType = res.data.data.jumpinner == null ? 2 : res.data.data.jumpinner;
                          this.formData.jumpLink = res.data.data.gotourl || '';
                          this.formData.title = res.data.data.title;
                          this.formData.abstract = res.data.data.summary;
                          this.formData.msgimgurl = res.data.data.imgurl;
                        } else {
                          callback(new Error('请输入站内信类型的id'));
                        }
                      } else {
      ...
      return {
        ...
        formDataRules: {
          pushId: [
            {required: true, validator: checkPushId, trigger: 'blur'}
          ]
        },
        formData: {
          pushId: '',//push Id
        ...
        tableOptions: [
          ...
          {
            label: '群发Id', prop: 'id', type: 'normal'
          },
          {
            label: '创建时间', prop: 'createTime', type: 'time',
          },
          {
            label: '群发时间', prop: 'expectTime', type: 'time',
          },
          {
            label: '群发对象', prop: 'type', type: 'filter', formatters: this.filterObj
          },
          {
            label: '类型', prop: 'smsType', type: 'filter', formatters: this.filterSmsType
          },
          {
            label: '摘要', prop: 'summary', type: 'normal'
          },
          {
            label: '摘要图片', prop: 'imgurl', type: 'img'
          },
          {
            label: '群发标题', prop: 'title', type: 'normal'
          },
          {
            label: '群发内容', prop: 'content', type: 'popover', popoverTitle: '查看'
          },
          {
            label: '群发数量', prop: 'smsTotal', type: 'normal'
          },
          {
            label: '下发成功数', prop: 'successTotal', type: 'normal'
          },
          {
            label: '下发失败数', prop: 'failTotal', type: 'normal'
          },
          {
            label: '状态', prop: 'status', type: 'filter', formatters: this.filterStatus
          }
        ],//所有表头
        exportLoading: false,
      }
    ...
    methods: {
      ...
      exportList() {
        ...
        let url = this.apiUrl + '/opt/smsmass/exportAppMessageList?status=' + this.checkStatus + '&sendType=' + this.listPushObject + '&keyword=' + this.searchList;
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$prompt('请输入测试手机号码', '提示', {
              ...
              this.formLoading = true;
              api.auSendAppMessageTest({
                messageid: this.formData.pushId,
                mobile: value,
                msgtype: this.bulkData.businessType
              }).then((res) => {
                if (res.data.code == 200) {
                  this.$message({
                    type: 'success',
