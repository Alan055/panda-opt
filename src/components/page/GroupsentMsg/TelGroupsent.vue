        ...
        <el-tabs type="border-card">
          <el-tab-pane label="新建群发">
            ...
            <div class="plugins-tips">
              为保障用户体验，自动电话群发请务必严谨，下发前请先内部测试，确认无误后再对用户进行批量下发。晚上10:00至早上8:00禁止对用户下发。
            </div>
            <div v-loading="formLoading">
              ...
              <div class="content-form">
                ...
                <el-form ref="formData" :model="formData" :rules="formDataRules" label-width="120px">
                  <!--  输入自动电话 ID-->
                  <el-form-item label="自动电话ID：" prop="pushId">
                    <el-input
                      type="text"
                      placeholder="请输入自动电话 ID"
                      v-model="formData.pushId">
                    </el-input>
                  </el-form-item>

                  <el-form-item label="自动电话音频：">
                    <el-input
                      type="input"
                      disabled
                      placeholder="输入自动电话Id 自动填充"
  ...
  export default {
    ...
    data() {
      var checkPushId = (rule, value, callback) => {
        let _this = this;
        if (!value) {
          return callback(new Error('自动电话id不能为空'));
        }

        if (value.length < 18) {
          callback(new Error('请输入正确长度的自动电话 Id'));
        } else {
          ...
          api.msGetSysEventDetailById({
            ...
            if (res.data.code == 200) {
              if (res.data.data) {
                if (res.data.data.messagetype == '4') {
                  this.Api.msGetSysEventDetailById({
                    ...
                    if (res.data.code == 200) {
                      if (res.data.data) {
                        if (res.data.data.messagetype == '4') {
                          this.formData.content = res.data.data.templateid;
                          this.formData.title = res.data.data.title;
                        } else {
                          callback(new Error('请输入自动电话类型的id'));
                        }
                      } else {
      ...
      return {
        ...
        tableOptions: [
          ...
          {
            label: '群发对象', prop: 'sendType', type: 'filter', formatters: this.filterObj
          },
          {
            label: '类型', prop: 'type', type: 'filter', formatters: this.filterSmsType
          },
          {
            label: '音频Id', prop: 'templeteId', type: 'normal'
          },
          {
            label: '备注', prop: 'title', type: 'content'
          },
          {
            label: '群发数量', prop: 'sendTotal', type: 'normal'
          },
          {
    ...
    methods: {
      ...
      exportList() {
        ...
        let url = apiUrl + '/opt/smsmass/exportAutotelMessageList?status=' + this.checkStatus + '&sendType=' + this.listPushObject + '&keyword=' + this.searchList;
      ...
      sendApply(formName) {
        ...
        if (valid_common && valid_common) {
          ...
          api.sentSaveAutotelBatchInfo(params).then((res) => {
            if (res.data.code == 200) {
