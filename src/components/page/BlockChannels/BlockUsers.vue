...
<template>
     <div class="blockUsers-view">
        <div class="container">
            <div class="plugins-tips">
                ...
                <div  class="add-btn" @click="addChannel">
                    <i class="el-icon-lx-roundadd"></i>新增屏蔽账号
                </div>
            </div>
            <el-table
        ...
        <el-dialog title="新增手机号屏蔽" width="40%" center  class="addChannelDialog" :visible.sync="addChannelDialog"  @closed="closeAddChannel">
            <el-row :gutter="40">
                        <el-form>
                            <el-table
        ...
        <el-dialog title="编辑屏蔽商业渠道" width="40%" center  :visible.sync="editChannelDialog"  @closed="closeDialog">
            <el-form ref="editChannelForm" :model="editChannelForm" :rules="editChannelRules" label-width="100px" size="mini">
                <el-form-item label="手机号码">
                    <el-input  readOnly :value="formatPhoneNumber(editChannelForm)"></el-input>
                </el-form-item>
                ...
                <el-form-item label="备注">
                    <el-input   v-model="editChannelForm.remark"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
        ...
        methods: {
          ...
          formatPhoneNumber(row){
            ...
            getLoanmarketListById: function(){
              this.Api.loGetUserLoanmarketListById({
            ...
            deleteChannel: function(row){
                this.$confirm('此操作将永久删除手机号 "'+row.mobile+'" 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'error',
                    center: true
                }).then(() => {
                    this.Api.loDeleteUserLoanmarketById({
                        userid: row.userid,
                        id: row.id
                    }).then((res)=>{
                        if(res.data.code == 200){
            ...
            sureSaveChannel: function(){
                ...
                if(count == this.addChannelList.length){
                    this.Api.loInsertUserLoanmarketList({
            ...
            sureSaveEditChannel: function(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.Api.loSaveUserLoanmarketList({
                            id: this.editChannelForm.id,
                            model: this.editChannelForm.model,
                            mobile: this.editChannelForm.mobile,
                            remark: this.editChannelForm.remark
                        }).then((res)=>{
                            if(res.data.code == 200){
                                this.$message.success("保存成功");
                                this.getLoanmarketListById();
